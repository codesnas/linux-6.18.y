...
static int btmtk_sdio_set_sleep(void)
{
	...
	u8 mtksdio_packet_header[MTK_SDIO_PACKET_HEADER_SIZE] = {0};
	/* The vendor defaults (SDIO mode 0x3, 640 ms host durations) let
	 * the CR4 -- shared with the Wi-Fi function -- enter a sleep the
	 * ported 6.18 Wi-Fi driver cannot own back from, stalling the
	 * Wi-Fi command path seconds after Bluetooth setup. Use the
	 * on-device proven values instead: mode 0x5 with 1600 ms, which
	 * keeps the CR4 awake while Bluetooth stays functional.
	 */
	u8 mtksdio_wmt_cmd[11] = {1, 0x7A, 0xFC, 7,
		/*3:sdio, 5:usb*/05,
		/*host non sleep duration*/0x40, 0x06,
		/*host non sleep duration*/0x40, 0x06, 0x0, 0x00};
